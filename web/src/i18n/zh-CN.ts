export default {
  ...
  settings: {
    ...
    irSection: {
      newRemoteTitle: '新建遥控',
      newRemoteDesc: '先创建一个遥控（如“客厅电视”），再在下方遥控库中学习它的按键',
      newRemotePlaceholder: '输入遥控名称，如：客厅电视',
      createRemote: '创建遥控',
      learnButton: '学习按键',
      buttonNamePlaceholder: '按键名称，如：电源',
      startLearn: '开始学习',
      cancelLearn: '取消学习',
      stateWaiting: '等待红外信号…请按住遥控器按键',
      stateSaved: '保存成功',
      stateFailed: '学习失败',
      errorButtonName: '请先填写按键名称',
      errorTimeout: '学习超时：未捕捉到红外信号',
      errorFailed: '学习过程中发生错误',
      noRemotesYet: '遥控库为空，先从上方“新建遥控”或“导入遥控包”开始。',
      noButtonsInRemote: '该遥控下暂无按键',
      sendTest: '发射测试',
      slotBind: '绑定槽位',
      slotNone: '不绑定',
      slotN: '槽位 {n}',
      slotPending: '{n} 处槽位修改未保存',
      slotSave: '保存修改',
      slotRevert: '撤销',
      slotSaved: '槽位修改已保存',
      kvmActive: 'KVM 切换有效',
      kvmMakeActive: '设为 KVM 切换',
      kvmSetDone: '已将「{name}」设为 KVM 切换遥控',
      slotOnlyKvm: '仅 KVM 切换遥控可绑定槽位',
      confirmDeleteRemote: '确定删除遥控「{name}」及其全部按键吗？',
      confirmDeleteButton: '确定删除该按键吗？',
      libraryTitle: '遥控库',
      libraryDesc: '学习到的遥控与按键：发射测试、绑定槽位、改名、删除与导入分享',
      export: '导出遥控包',
      importPack: '导入遥控包',
      importResult: '导入完成：{remotes} 个遥控、{buttons} 个按键（跳过重复 {skipped} 个）',
      libraryHint: '遥控包为 JSON 文件（*.onekvm-ir.json），包含一个遥控的全部按键码；每个遥控可通过下载图标导出分享。',
      hwTitle: '硬件设置',
      hwDesc: '发射 GPIO 与学习参数',
      hwReceiver: '红外接收',
      hwTransmitter: '红外发射',
      hwLed: '状态灯 (WS2812)',
      enableFeature: '启用红外遥控功能',
      rxDevice: '接收设备',
      txMode: '发射方式',
      txModeAuto: '自动',
      txModeLirc: '内核 LIRC 设备',
      txModeGpio: 'GPIO 直接驱动',
      txModeNone: '禁用发射',
      txGpioChip: '发射 GPIO 控制器',
      txGpioLine: '发射 GPIO 线号',
      carrier: '载波频率 (Hz)',
      learnTimeout: '学习超时 (毫秒)',
      ledEnabled: '启用 STA 状态灯',
      ledBrightness: '状态灯亮度',
      ledHint: '学习等待=闪绿，保存成功=蓝，操作失败=红',
    },
    environment: '环境',
