...
/** Infrared remote hardware and behavior settings (hand-synced with src/config/schema/ir.rs) */
export interface IrConfig {
	...
	tx_gpio_line: number;
	tx_mmap_base: number;
	tx_mmap_oen_offset: number;
	tx_mmap_out_offset: number;
	tx_bit: number;
	carrier: number;
	learn_timeout_ms: number;
	led_enabled: boolean;
	led_mmap_base: number;
	led_oen_offset: number;
	led_out_offset: number;
	led_bit: number;
	led_brightness: number;
}

export interface IrConfigUpdate {
	enabled?: boolean;
	rx_device?: string;
	tx_mode?: string;
	tx_gpio_chip?: string;
